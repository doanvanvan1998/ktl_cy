...
class coolsms
{
	...
	public function __construct($api_key, $api_secret, $basecamp=false)
	{
		if($basecamp)
		{
			$this->coolsms_user = $api_key; // coolsms_user라는 인스턴스를 생성
			$this->basecamp = true;
		}
		else
		{
	...
	{
		$ch = curl_init(); //  다양한 프로토콜로 데이터 전송이 가능한 Command line Tool
		...
		curl_setopt($ch, CURLOPT_URL, $host); // url 지정
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE); // 요청 결과를 문자열로 반환
		curl_setopt($ch, CURLOPT_SSLVERSION,3); // SSL 버젼 (https 접속시에 필요)
		curl_setopt($ch, CURLOPT_HEADER, 0); // 헤더 출력 여부
		curl_setopt($ch, CURLOPT_POST, $this->method); // Post Get 접속 여부, true라면 post 전송

		// Set POST DATA
		if($this->method)
		{
			curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type:multipart/form-data"));
			curl_setopt($ch, CURLOPT_POSTFIELDS, $this->content); // post data
		}
