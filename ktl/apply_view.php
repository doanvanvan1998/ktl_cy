  ...
  <div class="wrap">
    ...
    <div class="contents_wrap">
      <div class="container">
        ...
        <div class="step flex" style='border:1px solid #e0e0e0;margin-bottom:30px;'>
          <?php
          $nSubAdminCheck = 0;
          $TotalScore = 0;
          $query="select id,username from recruit_able_subadmin";
          $SubAdminResult = mysqli_query($con,$query);
          while($SubAdminRow = mysqli_fetch_array($SubAdminResult))
          {
            $query="select feedback,score from recruit_able_score where step='$step' and userid='$Id' and subadmin='$SubAdminRow[0]'";
            $SubAdminResult2 = mysqli_query($con,$query);
            $SubAdminRow2 = mysqli_fetch_array($SubAdminResult2);

            if($SubAdminRow2[0] == "") { $SubAdminRow2[0] = "미입력"; }
            if($SubAdminRow2[1] == "") { $SubAdminRow2[1] = "미입력"; }
            else
            {
              $SubAdminRow2[1] = $SubAdminRow2[1]."점";
            }

            echo "<div style='padding:15px;font-size:16px;'>[ ".$SubAdminRow[1]."면접관님 ]<br><br>
              <input type='text' id='user_score' disabled placeholder='점수를 입력하시기 바랍니다.' value='$SubAdminRow2[1]' style='font-size:16px;width:100%;border:1px solid grey;margin-bottom:10px;padding:10px;'/><br>
              <textarea name='feedback' id='feedback' disabled placeholder='피드백을 입력하시기 바랍니다.' style='padding:10px;width:100%;font-size:16px;border:1px solid grey;margin-bottom:20px;width:100%;height:200px;'>$SubAdminRow2[0]</textarea>
            </div>";
            $nSubAdminCheck++;

            $TotalScore += $SubAdminRow2[1];
          }
          if($nSubAdminCheck == 0)
          {
            echo "<div style='padding:10px;font-size:16px;'>아직 면접관의 피드백이 존재하지 않습니다.</div>";
          }
          ?>

        </div>
        <div style='float:right;color:red;font-weight:700;'>면접관 점수 : <?=$TotalScore?>점</div>
